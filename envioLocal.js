function enviarEditaisPorLocalizacao() {
  ...
  var respCheckbox = ui.alert(
    'Confirmação de Envio (Modo Restrito por DDD)',
    '📧 COTA DO GOOGLE: Você ainda tem ' + cotaGoogle + ' envios disponíveis hoje.\n\nDeseja enviar os editais marcados?\n\nO robô usará a Coluna G (Estado) para filtrar rigorosamente o DDD do fornecedor.',
    ui.ButtonSet.YES_NO
  );
  ...
  if (cotaGoogle <= 0) {
    ui.alert("🚨 Limite Atingido!\n\nA sua conta do Google não tem mais envios disponíveis hoje. Tente novamente em 24 horas.");
    return;
  }
  ...
  function obterTermosDeEstado(celula) {
    ...
    var siglas = String(celula).toUpperCase().split(/[,;]+/).map(function(s) {
      return s.trim();
  ...
  // =========================================================================
  // MATCH ESTRITO — keyword e texto precisam ter os mesmos tokens,
  // sem sobras em nenhum dos lados.
  // =========================================================================
  ...
  // =========================================================================
  // MATCH POR LINHAS — testa cada linha do produto individualmente.
  // Resolve o caso de fornecedores com múltiplos produtos separados por \n.
  // =========================================================================
  ...
  var emailsProcessadosNestaRodada = [];
  ...
  for (var k = 0; k < todasAbas.length; k++) {
    ...
    for (var i = 0; i < dadosFornecedores.length; i++) {
      ...
      for (var j = 0; j < editaisProcessados.length; j++) {
        ...
        // --- FILTRO DE ESTADO (restrito por DDD) ---
        if (edital.termosEstadoAlvo.length > 0) {
          if (!estadoPorDDD || edital.termosEstadoAlvo.indexOf(estadoPorDDD) === -1) {
            continue;
          }
        }
        ...
        // =====================================================================
        // MATCH DE PALAVRAS-CHAVE — testa linha a linha
        // =====================================================================
        ...
        for (var r = 0; r < edital.keywordTokens.length; r++) {
          ...
          if (temMatchEmAlgumaLinha(tokens, categoriaMestraBruta)) {
            temMatchKeyword = true;
            break;
          }
        }
      ...
      var destinatarioReal = listaEmails[0];

      if (emailsProcessadosNestaRodada.indexOf(destinatarioReal) > -1) {
        novosStatus.push(["Duplicado (Ignorado)"]);
        continue;
      }
      ...
      try {
        ...
        houveAlteracao = true;
        sentCount++;
        emailsProcessadosNestaRodada.push(destinatarioFinal);
